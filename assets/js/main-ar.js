...
function handleIgnition() {
    state = 1;
    startBtn.innerText = "انتظر لمدة ثلاثين ثانية...";
    updateSubtitle("دع طرف العود يشتعل بالتساوي، وشكّل جمرة حوله "
        + "سيساعد ذلك على إطلاق الرائحة بشكل أكثر فعالية. ثم أطفئ اللهب بالنفخ برفق.");
    ...
    tl.to(startBtn, { pointerEvents: "none", opacity: 0.8, duration: 0.1 })
        ...
        .to(startBtn, {
            ...
            onStart: () => { startBtn.innerText = "انفخ"; }
        });
}

function handleBlow() {
    state = 2;
    startBtn.innerText = "أطفئ";
    updateSubtitle("اتركها تشتعل ببطء لمدة عشرين إلى ثلاثين دقيقة، وقم بنشر الرائحة عن طريق تحريك الدخان برفق بيدك."
        + " ثم قم بإطفاء العود عن طريق قلبه في القاعدة.");
...
function handleReset() {
    state = 0;
    startBtn.innerText = "إقلب ";
    updateSubtitle("ستبقى الرائحة الناتجة لمدة يوم أو أكثر.");
    startBtn.style.pointerEvents = "none";
    if (pulseTween) pulseTween.kill();
    const flipTL = gsap.timeline({
        onComplete: () => {
            startBtn.innerText = "اشعل";
