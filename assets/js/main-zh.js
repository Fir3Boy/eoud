...
function handleIgnition() {
    state = 1;
    startBtn.innerText = "请等待 30 秒...";
    updateSubtitle("让香尖均匀着火，并在香尖周围形成燃烧的火星。这有助于更有效地释放香气。随后，轻轻吹气熄灭火焰。");
    ...
    tl.to(startBtn, { pointerEvents: "none", opacity: 0.8, duration: 0.1 })
        ...
        .to(startBtn, {
            ...
            onStart: () => { startBtn.innerText = "吹一下顶端"; }
        });
}

function handleBlow() {
    state = 2;
    startBtn.innerText = "掐灭并熄灭";
    updateSubtitle("点燃后，让其阴燃 20 至 30 分钟，并用手轻轻扇动烟雾使香气扩散。随后，将香棒翻转插入底座，熄灭顶端。");
...
function handleReset() {
    state = 0;
    startBtn.innerText = "翻转香棒";
    updateSubtitle("产生的余香将缭绕整日，甚至更久。");
    startBtn.style.pointerEvents = "none";
    if (pulseTween) pulseTween.kill();
    const flipTL = gsap.timeline({
        onComplete: () => {
            startBtn.innerText = "点燃";
